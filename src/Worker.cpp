...
void Worker::run() {
  ...
  while (!m_isStopped) {
    ...
    Crypto::cn_slow_hash(context, localJob.blob.data(), localJob.blob.size(), hash, NodeAdapter::instance().getCurrentBlockMajorVersion() < CryptoNote::CURRENT_BLOCK_MAJOR + 2 ? 0 : 1);
    ++m_hashCounter;
